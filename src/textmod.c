...
static unsigned long sys_listen = 0;
...
static void textmod_enable(void)
{
    ...
    memcpy((void*)sys_listen, call_data, 5);
    protect_memory(cr0);
...
static void textmod_disable(void)
{
    ...
    // Restore sys_listen
    cr0 = unprotect_memory();
    memcpy((void*)sys_listen, orig_data, 5);
    protect_memory(cr0);
...
void textmod_init(void)
{
    ...
    sys_listen = kernel_symbol_lookup("__sys_listen");
    if (!sys_listen) {
    ...
    offset = textmod_listen - sys_listen - 5;
    memcpy(call_data+1, (void*)&offset, 4);

    // Save original data
    memcpy(orig_data, (void*)sys_listen, 5);
